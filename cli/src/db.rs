...
use super::define;
use enmap_core::model::{OuiData, PortData, OsTtl, TCPFingerprint};

pub fn get_oui_map() -> HashMap<String, String> {
    let mut oui_map: HashMap<String, String> = HashMap::new();
    let rs_nscan_oui: Vec<OuiData> = serde_json::from_str(define::ENMAP_OUI).unwrap_or(vec![]);
...
pub fn get_tcp_map() -> HashMap<String, String> {
    ...
    let rs_nscan_tcp_port: Vec<PortData> = serde_json::from_str(define::ENMAP_TCP_PORT).unwrap_or(vec![]);
...
pub fn get_default_ports() -> Vec<u16> {
    let rs_nscan_default_ports: Vec<&str> = define::ENMAP_DEFAULT_PORTS.trim().split("\n").collect();
...
pub fn get_http_ports() -> Vec<u16> {
    let rs_nscan_http_ports: Vec<&str> = define::ENMAP_HTTP.trim().split("\n").collect();
...
pub fn get_https_ports() -> Vec<u16> {
    let rs_nscan_https_ports: Vec<&str> = define::ENMAP_HTTPS.trim().split("\n").collect();
...
/* pub fn get_os_fingerprints() -> Vec<OSFingerprint> {
    let fingerprints: Vec<OSFingerprint> = serde_json::from_str(define::ENMAP_OS).unwrap_or(vec![]);    
    fingerprints
} */

pub fn get_tcp_fingerprints() -> Vec<TCPFingerprint> {
    let fingerprints: Vec<TCPFingerprint> = serde_json::from_str(define::ENMAP_OSDB).unwrap_or(vec![]);    
    fingerprints
}

pub fn get_os_ttl() -> HashMap<u8, String> {
    let mut ttl_map: HashMap<u8, String> = HashMap::new();
    let rs_os_ttl: Vec<OsTtl> = serde_json::from_str(define::ENMAP_OS_TTL).unwrap_or(vec![]);
    for os_ttl in rs_os_ttl {
