...
pub fn show_default_interface(args: &ArgMatches) {
    ...
    match args.get_one::<PathBuf>("save") {
        Some(file_path) => {
            match nerum_core::fs::save_text(file_path, serde_json::to_string_pretty(&iface).unwrap()) {
                Ok(_) => {
...
pub fn show_interfaces(args: &ArgMatches) {
    ...
    match args.get_one::<PathBuf>("save") {
        Some(file_path) => {
            match nerum_core::fs::save_text(file_path, serde_json::to_string_pretty(&interfaces).unwrap()) {
                Ok(_) => {
                    output::log_with_time(&format!("Saved to {}", file_path.to_string_lossy()), "INFO");
                },
                Err(e) => {
