    ...
    for response in &trace_result.nodes {
        match response.probe_status.kind {
            ProbeStatusKind::Done => {
                ...
                response_tree.push(node_label("NodeType", Some(&response.node_type.name()), None));
                responses_tree.push(response_tree);
            },
            _ => {
                let mut response_tree = Tree::new(node_label("Sequence", Some(response.seq.to_string().as_str()), None));
                response_tree.push(node_label("Status", Some(&response.probe_status.kind.name()), None));
                response_tree.push(node_label("Message", Some(&response.probe_status.message), None));
                responses_tree.push(response_tree);
            }
        }
    }
    tree.push(responses_tree);
    tree.push(node_label("Status", Some(&trace_result.probe_status.kind.name()), None));
    println!("{}", tree);
}
